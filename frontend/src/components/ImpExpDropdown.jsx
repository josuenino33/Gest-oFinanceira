...
export default function ImpExpDropdown({ tipo, lista, colunasCsv, nomeArquivo, onImportado }) {
  ...
  return (
    <>
      ...
      <div ref={dropdownRef} className='relative'>
        <button
          onClick={() => setOpen(p => !p)}
          className='border px-4 py-2 rounded-xl text-sm font-semibold hover:opacity-80 transition whitespace-nowrap flex items-center gap-1'
        ...
        {open && (
          <div
            className='absolute right-0 top-full mt-1 z-50 rounded-xl border shadow-xl py-1 min-w-[160px]'
            style={{ background: 'var(--bg-card)', borderColor: 'var(--border-color)' }}
          >
            <button
              onClick={() => { setOpen(false); inputRef.current?.click() }}
              className='w-full text-left px-4 py-2.5 text-sm hover:opacity-70 transition'
            ...
            <button
              onClick={() => { setOpen(false); exportToCSV(lista, nomeArquivo, colunasCsv) }}
              className='w-full text-left px-4 py-2.5 text-sm hover:opacity-70 transition'
              style={{ color: 'var(--text-muted)' }}
            >
              ⬇ Exportar CSV
      ...
      >
        <div className='space-y-4'>
            ...
            {['descricao', 'valor', 'data'].map(campo => (
              <div key={campo} className='space-y-1'>
                ...
                <select
                  ...
                  onChange={e => setMapeamento(p => ({ ...p, [campo]: Number(e.target.value) }))}
          ...
          <button
            onClick={importar}
            disabled={loading || !registrosMapeados.length}
